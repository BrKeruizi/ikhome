...
public class UserSign {
    ...
    public void getMySign(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(username == null){
            ...
            List<org.framework.ikhome.entity.UserSign> userSigns = userSService.getMySignNow(username, monthstr);
            if(userSigns.size() == 0){
                res = "{\"status\": \"valid\"}";
            }
            else{
                StringBuffer date = new StringBuffer(",");
                for (org.framework.ikhome.entity.UserSign userSign: userSigns) {
