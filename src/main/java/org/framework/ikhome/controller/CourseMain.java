...
import org.framework.ikhome.entity.UserMain;
import org.framework.ikhome.service.CourseMService;
import org.framework.ikhome.service.UserMService;
...
public class CourseMain {
    ...
    public void getCourseList(Integer stype, String ctype, String sort, Integer startpos, Integer status, String keyword,
        ...
        String res = null;
        List<org.framework.ikhome.entity.CourseMain> courseMains = null;
        //默认：不限进行排序
        if (ctype.equals("all")) {
            //时间最新排序
            if (sort.equals("new")) {
        ...
        if (courseMains.size() == 0) {
            ...
            res = "{";
            int i = 1;
            for (org.framework.ikhome.entity.CourseMain courseMain : courseMains) {
    ...
    public void getCourseType(String stype, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<org.framework.ikhome.entity.CourseMain> courseMains = courseMService.getCourseType(stype);
        if (courseMains.size() == 0) {
            res = "{\"status\": \"valid\", \"len\": \"0\"}";
        } else {
            res = "{";
    ...
    public void courseSearch(String keyword, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<org.framework.ikhome.entity.CourseMain> courseMains = courseMService.courseSearch(keyword);
        if (courseMains.size() == 0) {
            res = "{\"status\": \"valid\", \"len\": \"0\"}";
        } else {
            res = "{";
    ...
    public void getCourseById(Integer id, HttpServletResponse response) throws IOException {
        ...
        org.framework.ikhome.entity.CourseMain courseMain = courseMService.getCourseById(id);
        UserMain userMain = userMService.getByUser(courseMain.getUsername());
