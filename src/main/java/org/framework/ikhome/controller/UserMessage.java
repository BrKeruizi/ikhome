...
import org.framework.ikhome.service.UserMSService;
...
public class UserMessage {
    ...
    public void getMyMessage(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(username == null){
            ...
            //首先获取所有的我的通知数据
            List<org.framework.ikhome.entity.UserMessage> userMessageList = userMSService.getMyMessage(username);
            if(userMessageList.size() == 0){
                ...
                res = "{";
                int i = 1;
                for (org.framework.ikhome.entity.UserMessage userMessage: userMessageList) {
    ...
    public void getMessageByUser(String suser,HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(username == null){
            ...
            //首先获取所有的我的通知数据
            List<org.framework.ikhome.entity.UserMessage> userMessageList = userMSService.getMessageBySuser(suser, username);
            if(userMessageList.size() == 0){
                res = "{\"status\": \"valid\"}";
            }
            else {
    ...
    public void getMessageByStatus(String suser, String status, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(username == null){
            ...
            List<org.framework.ikhome.entity.UserMessage> userMessages = userMSService.getMessageByStatus(suser, username, sta);
            if(userMessages.size() == 0){
                ...
                int i = 1;
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
                for (org.framework.ikhome.entity.UserMessage userMessage: userMessages) {
                    res += "\"" + i + "\": ";
                    String temp = "{\"status\": \"" + userMessage.getStatus() + "\", " +
                            "\"stime\": \"" + simpleDateFormat.format(userMessage.getStime()) + "\", " +
                            "\"id\": \"" + userMessage.getId() + "\", " +
